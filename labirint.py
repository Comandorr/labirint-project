    ...
    def control(self):
        ...
        if keyboard[K_a] and self.rect.x > 0:
            ...
            if sprite.spritecollide(self, walls, False):
                self.rect.x += 5
        # касание врага
        if self.rect.colliderect(e1.rect) or self.rect.colliderect(e2.rect):
            hp -= 1
            if hp == 0:
                game = 0
            self.rect.x = 100
...
s5 = Hero('brick.png', 400, 420)
s6 = Hero('brick.png', 800, 180)
s9 = Hero('brick.png', 400, 540)
...
heal = Hero('аптечка.png', 200, 500)
heal.resize(70, 70)

finish = Hero('портал.png', -1100, 700)
finish.resize(100, 100)

c1 = Hero('монетка.png', 1000, 200)
c2 = Hero('монетка.png', 1000, 600)
c3 = Hero('монетка.png', 200, 600)
coins = sprite.Group()
coins.add(c1, c2, c3)
for i in coins.sprites():
    i.resize(70, 70)

fon = image.load('фон.png')
fon = transform.scale(fon, (1200, 800))

heart = image.load('сердце.png')
heart = transform.scale(heart, (50, 50))

turret = Hero('турель.png', 100, 700)
...
while game == 1:
    ...
    if shoot_timer == 100:
        shoot_timer = 0
        b = Bullet('огонь.png', turret.rect.x, turret.rect.y, 10, 0)
        b.resize(50, 50)
        bullets.add(b)
    heal.show()
    finish.show()
